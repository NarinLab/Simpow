...
void scheduler_print(){
  long NOW = millis();
  if(NOW - SCHDULER_PRINT_LAST_RUN > 0){
    SCT013 sct013;
    // Gunakan variable reference sebagai parameter
    sensory_get_sensor(sct013);
  
    Log.notice(F("VOL:%F IRMS:%F WATT:%F ICAL:%F BRES:%F ANR:%d" CR), sct013.tegangan, sct013.arus, sct013.watt, sct013.ical, sct013.burden_res, analogRead(PIN_SENSOR_SCT013));
